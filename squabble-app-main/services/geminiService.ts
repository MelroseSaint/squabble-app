...
import { generateMockFighters } from './mockGenerator';
...
export const generateTrashTalk = async (apiKey: string, fighter: Fighter, history: Message[]): Promise<string> => {
  const ai = new GoogleGenAI({ apiKey });

  try {
    const response = await ai.models.generateContent({
      model: "gemini-1.5-flash",
      contents: `User said: "${history[history.length - 1].text}". History: ${JSON.stringify(history)}. Reply as ${fighter.name}. You are angry, aggressive, and want to fight. Suggest a location like Walmart or a parking lot. Keep it short (under 20 words).`,
      config: {
        systemInstruction: `You are playing the character ${fighter.name} on an app called Squabble (Tinder for fighting). Your bio is: ${fighter.bio}. You are extremely aggressive but in a funny way. Generate a creative and funny trash talk line.`,
        maxOutputTokens: 100,
      }
    });
...
export const generateChatReply = async (apiKey: string, fighter: Fighter, history: Message[], trashTalk: boolean = false): Promise<string> => {
  if (trashTalk) {
    return generateTrashTalk(apiKey, fighter, history);
  }
  if (!apiKey) {
