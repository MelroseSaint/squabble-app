...
var Emitter = class {
  ...
  async subscribeOnce(event, historic = false) {
    ...
    if (historic && this.collectable[event]) {
      ...
      if (((_b = this.collectable[event]) == null ? void 0 : _b.length) === 0 && delete this.collectable[event], args)
        return args;
    }
...
var UUID = class _UUID {
  ...
  static parse(uuid) {
    ...
    if (hex) {
      ...
      for (let i = 0; i < 16; i += 4) {
        ...
        inner[i + 0] = n >>> 24, inner[i + 1] = n >>> 16, inner[i + 2] = n >>> 8, inner[i + 3] = n;
      }
  ...
  toString() {
    ...
    for (let i = 0; i < this.bytes.length; i++)
      text += DIGITS.charAt(this.bytes[i] >>> 4), text += DIGITS.charAt(this.bytes[i] & 15), (i === 3 || i === 5 || i === 7 || i === 9) && (text += "-");
    return text;
  }
  toHex() {
    let text = "";
    for (let i = 0; i < this.bytes.length; i++)
      text += DIGITS.charAt(this.bytes[i] >>> 4), text += DIGITS.charAt(this.bytes[i] & 15);
    return text;
  }
  toJSON() {
    return this.toString();
...
var V7Generator = class {
  constructor(randomNumberGenerator) {
    this.timestamp = 0, this.counter = 0, this.random = randomNumberGenerator ?? getDefaultRandom();
  }
  generate() {
  ...
  generateOrResetCore(unixTsMs, rollbackAllowance) {
    ...
    return value === void 0 && (this.timestamp = 0, value = this.generateOrAbortCore(unixTsMs, rollbackAllowance)), value;
  }
  ...
  generateV4() {
    let bytes = new Uint8Array(Uint32Array.of(this.random.nextUint32(), this.random.nextUint32(), this.random.nextUint32(), this.random.nextUint32()).buffer);
    return bytes[6] = 64 | bytes[6] >>> 4, bytes[8] = 128 | bytes[8] >>> 2, UUID.ofInner(bytes);
  }
};
...
var StringRecordId = class _StringRecordId extends Value {
  constructor(rid) {
    ...
    if (rid instanceof _StringRecordId)
      this.rid = rid.rid;
...
function jsonify(input) {
  if (typeof input == "object") {
    if (input === null)
      return null;
    if (input instanceof Date || input instanceof Uuid || input instanceof Decimal || input instanceof Duration || input instanceof Future || input instanceof Range || input instanceof StringRecordId || input instanceof RecordIdRange || input instanceof RecordId || input instanceof Geometry || input instanceof Table)
      return input.toJSON();
    switch (Object.getPrototypeOf(input)) {
      case Object.prototype: {
...
function processAuthVars(vars, fallback) {
  if ("scope" in vars || "access" in vars && "variables" in vars && vars.variables) {
    if (!vars.namespace) {
      if (!(fallback == null ? void 0 : fallback.namespace))
        throw new NoNamespaceSpecified();
      vars.namespace = fallback.namespace;
    }
    if (!vars.database) {
      if (!(fallback == null ? void 0 : fallback.database))
...
var EngineAuth = class extends AuthController {
  ...
  rpc(method, params) {
    if (!this.connection)
      throw new NoActiveSocket();
    return this.connection.rpc({ method, params }, true);
  }
};
...
var HttpEngine = class extends AbstractRemoteEngine {
  ...
  async rpc(request, force) {
    var _a, _b;
    if (force || await this.ready, !this.connection.url)
...
var WebsocketEngine = class extends AbstractRemoteEngine {
  ...
  async connect(url) {
    this.connection.url = url, (async () => {
      ...
      for (; this.connection.url; )
        if (initial)
          initial = false, this.setStatus("connecting"), this.ready = this.createSocket(), await this.disconnected.promise;
        else {
  ...
  async createSocket() {
    ...
    socket.addEventListener("open", async () => {
      var _a, _b, _c;
      this.socket = socket, await ((_b = (_a = this.context).prepare) == null ? void 0 : _b.call(_a, new EngineAuth(this))), this.setStatus("connected"), (_c = this.pinger) == null ? void 0 : _c.stop(), this.pinger = new Pinger(3e4), this.pinger.start(() => {
        try {
  ...
  async rpc(request, force) {
    if (force || await this.ready, !this.socket)
      throw new ConnectionUnavailable();
    let res;
    for (; !res; ) {
      let id2 = getIncrementalID(), response = this.emitter.subscribeOnce(`rpc-${id2}`);
      if (this.socket.send(this.encodeCbor({ id: id2, ...request })), force && this.socket.readyState === WebSocket2.CLOSED)
        throw new EngineDisconnected();
      let [raw] = await response;
      if (raw instanceof EngineDisconnected)
        throw raw;
...
surrealdb/dist/index.bundled.mjs:
  (*! Bundled license information:
