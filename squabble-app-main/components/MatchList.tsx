...
export const MatchList: React.FC<MatchListProps> = ({ matches, onSelectMatch, onDeleteMatch }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-transparent text-fb-text p-4 overflow-y-auto relative animate-in fade-in">
      <h2 className="text-2xl font-bold text-fb-text mb-6">Friends</h2>

      {matches.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 text-fb-text-secondary">
          <p>No friends added yet.</p>
          <p className="text-sm">Find people on Home.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {matches.map((match) => (
            <div
              ...
              className="flex items-center gap-4 p-3 rounded-lg hover:bg-fb-hover transition-colors cursor-pointer"
            >
              <div className="relative">
                ...
                />
                <div className="absolute bottom-0 right-0 w-4 h-4 bg-green-500 rounded-full border-2 border-fb-card"></div>
              </div>

              <div className="flex-1">
                <h3 className="text-lg font-semibold text-fb-text">{match.fighter.name}</h3>
                <p className="text-sm text-fb-text-secondary truncate">{match.lastMessage || "Say hello..."}</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 text-fb-text-secondary hover:bg-fb-hover rounded-full transition-colors"
                  title="Unfriend"
                >
                  <Trash2 size={18} />
                </button>
                <MessageSquare className="text-fb-blue" size={20} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-squabble-dark border border-gray-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-2xl font-heading font-bold text-white uppercase mb-2 animate-laser-pulse text-shadow-laser">Cancel Fight?</h3>
