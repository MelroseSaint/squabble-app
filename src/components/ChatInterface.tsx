...
import { Send, ChevronLeft, Video, Flag, MoreVertical, ShieldAlert } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-black relative">
      ...
      <div className="flex items-center justify-between p-4 bg-squabble-dark border-b border-gray-800 relative z-20">
        ...
        <div className="flex flex-col items-center">
           <span className="font-heading font-bold text-xl uppercase tracking-wider">{match.fighter.name}</span>
        ...
        <div className="flex items-center">
            <button 
                ...
                className="p-2 text-squabble-red mr-1 hover:text-white transition-colors"
            >
                <Video size={24} />
            </button>
        ...
        {showMenu && (
            <div className="absolute top-16 right-4 bg-gray-900 border border-gray-700 rounded-xl shadow-xl w-48 overflow-hidden z-30">
                <button 
                    ...
                    className="w-full text-left px-4 py-3 hover:bg-gray-800 flex items-center gap-2 text-red-500 font-bold uppercase text-xs"
                >
                    <Flag size={14} /> Report User
                </button>
                <button 
                    onClick={handleUnmatch}
                    className="w-full text-left px-4 py-3 hover:bg-gray-800 flex items-center gap-2 text-gray-400 font-bold uppercase text-xs"
                >
      ...
      <div className="p-4 bg-squabble-dark border-t border-gray-800 relative z-20">
        <div className="flex items-center gap-2">
          ...
          <button 
            onClick={handleSend}
            className="p-3 bg-squabble-red rounded-full text-white hover:bg-red-700 transition-colors"
          >
            <Send size={20} />
          </button>
        </div>
